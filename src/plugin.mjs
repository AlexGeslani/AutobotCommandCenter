  ...
  function ProjectPortfolio({ route, go, portfolio }) {
    ...
    return h('div', { className: 'acc-view' },
      ...
      h('section', { className: 'acc-portfolio-group acc-focus-board', 'aria-labelledby': 'acc-focus-title' },
        h('div', { className: 'acc-section-heading' }, h('div', null, h('p', { className: 'acc-eyebrow' }, portfolio.policy.activeLimit == null ? 'In flight' : 'One in, one out'), h('h3', { id: 'acc-focus-title' }, portfolio.policy.activeLimit == null ? `Active / in flight · ${activeProjects.length}` : `Current focus · ${activeProjects.length}/${portfolio.policy.activeLimit}`)), h('small', null, portfolio.policy.rule)),
